...
class LatexMathTokenizer extends Tokenizer {
    ...
    private void createPatterns() throws ParserCreationException {
        ...
        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_FLEQN,
                                   "DOC_OPT_FLEQN",
                                   TokenPattern.STRING_TYPE,
                                   "fleqn");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_LEQNO,
                                   "DOC_OPT_LEQNO",
                                   TokenPattern.STRING_TYPE,
                                   "leqno");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_DRAFT,
                                   "DOC_OPT_DRAFT",
                                   TokenPattern.STRING_TYPE,
                                   "draft");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_LANDSCAPE,
                                   "DOC_OPT_LANDSCAPE",
                                   TokenPattern.STRING_TYPE,
                                   "landscape");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_FONT_10,
                                   "DOC_OPT_FONT_10",
                                   TokenPattern.STRING_TYPE,
                                   "10pt");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_FONT_11,
                                   "DOC_OPT_FONT_11",
                                   TokenPattern.STRING_TYPE,
                                   "11pt");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_FONT_12,
                                   "DOC_OPT_FONT_12",
                                   TokenPattern.STRING_TYPE,
                                   "12pt");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_PAPER_LETTER,
                                   "DOC_OPT_PAPER_LETTER",
                                   TokenPattern.STRING_TYPE,
                                   "letterpaper");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_PAPER_A4,
                                   "DOC_OPT_PAPER_A4",
                                   TokenPattern.STRING_TYPE,
                                   "a4paper");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_PAPER_A5,
                                   "DOC_OPT_PAPER_A5",
                                   TokenPattern.STRING_TYPE,
                                   "a5paper");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_PAPER_B5,
                                   "DOC_OPT_PAPER_B5",
                                   TokenPattern.STRING_TYPE,
                                   "b5paper");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_PAPER_EXECUTIVE,
                                   "DOC_OPT_PAPER_EXECUTIVE",
                                   TokenPattern.STRING_TYPE,
                                   "executivepaper");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_PAPER_LEGAL,
                                   "DOC_OPT_PAPER_LEGAL",
                                   TokenPattern.STRING_TYPE,
                                   "legalpaper");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_COLUMN_ONE,
                                   "DOC_OPT_COLUMN_ONE",
                                   TokenPattern.STRING_TYPE,
                                   "onecolumn");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_COLUMN_TWO,
                                   "DOC_OPT_COLUMN_TWO",
                                   TokenPattern.STRING_TYPE,
                                   "twocolumn");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_PAGE_TITLE,
                                   "DOC_OPT_PAGE_TITLE",
                                   TokenPattern.STRING_TYPE,
                                   "titlepage");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_PAGE_NOTITLE,
                                   "DOC_OPT_PAGE_NOTITLE",
                                   TokenPattern.STRING_TYPE,
                                   "notitlepage");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_SIDE_ONE,
                                   "DOC_OPT_SIDE_ONE",
                                   TokenPattern.STRING_TYPE,
                                   "oneside");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_SIDE_TWO,
                                   "DOC_OPT_SIDE_TWO",
                                   TokenPattern.STRING_TYPE,
                                   "twoside");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_OPEN_ANY,
                                   "DOC_OPT_OPEN_ANY",
                                   TokenPattern.STRING_TYPE,
                                   "openright");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_OPT_OPEN_RIGHT,
                                   "DOC_OPT_OPEN_RIGHT",
                                   TokenPattern.STRING_TYPE,
                                   "openright");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_ARTICLE,
                                   "DOC_CLASS_ARTICLE",
                                   TokenPattern.STRING_TYPE,
                                   "article");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_IEEE,
                                   "DOC_CLASS_IEEE",
                                   TokenPattern.STRING_TYPE,
                                   "IEEEtran");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_PROC,
                                   "DOC_CLASS_PROC",
                                   TokenPattern.STRING_TYPE,
                                   "proc");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_MINIMAL,
                                   "DOC_CLASS_MINIMAL",
                                   TokenPattern.STRING_TYPE,
                                   "minimal");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_REPORT,
                                   "DOC_CLASS_REPORT",
                                   TokenPattern.STRING_TYPE,
                                   "report");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_BOOK,
                                   "DOC_CLASS_BOOK",
                                   TokenPattern.STRING_TYPE,
                                   "book");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_SLIDES,
                                   "DOC_CLASS_SLIDES",
                                   TokenPattern.STRING_TYPE,
                                   "slides");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_MEMOIR,
                                   "DOC_CLASS_MEMOIR",
                                   TokenPattern.STRING_TYPE,
                                   "memoir");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_LETTER,
                                   "DOC_CLASS_LETTER",
                                   TokenPattern.STRING_TYPE,
                                   "letter");
        addPattern(pattern);

        pattern = new TokenPattern(LatexMathConstants.DOC_CLASS_BEAMER,
                                   "DOC_CLASS_BEAMER",
                                   TokenPattern.STRING_TYPE,
                                   "beamer");
        addPattern(pattern);
